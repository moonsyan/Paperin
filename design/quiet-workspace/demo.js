...
try {
    ...
    if (stored) {
        if (stored.length > 2_000_000)
            throw new Error('演示缓存超限');
        const parsed = JSON.parse(stored);
        if (!isRecord(parsed) || !Array.isArray(parsed.documents) || parsed.documents.length === 0 || parsed.documents.length > 50 || !parsed.documents.every(isDocument))
            throw new Error('演示缓存格式错误');
        const documents = parsed.documents;
        if (new Set(documents.map((doc) => doc.id)).size !== documents.length)
            throw new Error('文档身份重复');
        const activeId = documents.find((doc) => doc.id === parsed.activeId)?.id ?? documents[0].id;
        const tabs = Array.isArray(parsed.tabs) ? parsed.tabs.filter((id) => typeof id === 'string' && documents.some((doc) => doc.id === id)) : [activeId];
...
const query = element('palette-query');
const systemTheme = matchMedia('(prefers-color-scheme: dark)');
const dialogs = Array.from(document.querySelectorAll('dialog'));
const collapsedGroups = new Set(['项目手记']);
const documentScroll = new Map();
let filter = 'all';
let saveTimer;
let toastTimer;
let headingObserver;
...
function renderNavigation() {
    ...
    if (documents.length === 0)
        tree.append(textElement('p', '还没有收藏。点击文档标题上方的星标，把喜欢的内容留在这里。', 'empty-result'));
...
function renderOutline() {
    ...
    for (let index = 0; index < headings.length; index++) {
        ...
        const entry = button('', `outline-item${index === 0 ? ' active' : ''}`, () => {
            ...
            if (innerWidth < 1024)
                workspace.classList.remove('outline-popover');
        });
    ...
    if (headings.length === 0)
        outline.append(textElement('p', '标题会出现在这里', 'margin-heading'));
    headingObserver = new IntersectionObserver((entries) => {
        const visible = entries.filter((entry) => entry.isIntersecting).sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top);
        if (visible.length === 0)
            return;
        outline.querySelectorAll('[data-heading]').forEach((entry) => {
...
function renderDocument() {
    ...
    for (const item of related)
        relatedLinks.append(button(item.title, '', () => openDocument(item.id)));
    if (!related.length)
        relatedLinks.append(textElement('span', '当前还没有同组笔记'));
    renderNavigation();
    renderOutline();
    updateStats();
    ...
    for (const child of Array.from(body.childNodes)) {
        if (child.nodeType === Node.TEXT_NODE) {
            if (child.textContent?.trim())
                blocks.push({ kind: 'p', text: child.textContent });
            continue;
        }
...
function closeTab(id) {
    ...
    if (state.activeId === id)
        openDocument(state.tabs[Math.min(index, state.tabs.length - 1)]);
    else {
        renderNavigation();
        persist();
    }
...
function newDocument() {
    if (state.documents.length >= 50) {
        toast('Demo 最多保留 50 篇文档，请先导出已有内容。');
        return;
    }
...
function closeDialogs() { dialogs.forEach((dialog) => { if (dialog.open)
    dialog.close(); }); }
function openDialog(id) {
    closeDialogs();
    element(id).showModal();
}
function applyPreferences() {
    ...
    document.querySelectorAll('[data-theme-choice]').forEach((node) => node.setAttribute('aria-pressed', String(node.dataset.themeChoice === preferences.theme)));
    element('font-choice').value = preferences.font;
    element('font-size').value = String(preferences.size);
...
function selectSearch(index) {
    ...
    element('palette-results').querySelectorAll('[role="option"]').forEach((node, position) => {
...
function renderSearch() {
    ...
    if (!searchResults.length)
        results.append(textElement('p', '没有找到匹配内容，试试另一个关键词。', 'empty-result'));
    selectSearch(0);
}
document.addEventListener('click', (event) => {
    ...
    if (action)
        actions[action]?.();
    const nextFilter = event.target.closest('[data-filter]')?.dataset.filter;
    if (nextFilter) {
        filter = nextFilter;
        renderNavigation();
    }
...
element('font-choice').addEventListener('change', (event) => {
    state.preferences.font = event.target.value === 'serif' ? 'serif' : 'sans';
    applyPreferences();
    persist();
});
element('font-size').addEventListener('input', (event) => {
    state.preferences.size = Number(event.target.value);
    applyPreferences();
    persist();
});
document.addEventListener('keydown', (event) => {
    if (event.isComposing || compositionActive || event.keyCode === 229)
        return;
    const command = event.ctrlKey || event.metaKey;
    const key = event.key.toLowerCase();
    if (command && (key === 'k' || key === 'p')) {
        event.preventDefault();
        actions.search();
        return;
    }
...
systemTheme.addEventListener('change', () => { if (state.preferences.theme === 'system')
    applyPreferences(); });
window.addEventListener('pagehide', () => { if (saveTimer)
    persist(); });
applyPreferences();
renderDocument();
if (restored)
    element('save-message').textContent = '已恢复此浏览器的笔记';
if (restoreError)
    toast('演示缓存无法读取，已载入示例；原缓存暂未覆盖。');
