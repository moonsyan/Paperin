...
- 输出：HTML、PDF、DOCX、EPUB、LaTeX、Pandoc 等导出入口保留在菜单、命令面板和文档操作中。导出 Markdown、HTML、PDF、Word 或 Pandoc 前会检查空图片、不安全链接和缺失的本地目标；未完成任务只提醒，不会被删掉。取消导出不会改动原文件。
...
- 工作区搜索结果可以插入到当前文章：带来源链接的片段是插入当时的快照，不会改来源文件；插入后回到打开搜索时的位置，可用撤销收回。换成另一篇后再插入会被拒绝。
- 再次打开同一知识库时，会填回上次的搜索词。阅读位置随文档视图保存，不另存正文。打开后若索引没扫完、附件缺失或链接对不上，只给出说明，不改原文件。
