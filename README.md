...
- 输出：HTML、PDF、DOCX、EPUB、LaTeX、Pandoc 等导出入口保留在菜单、命令面板和文档操作中。导出 Markdown 或 Pandoc 前会检查空图片、不安全链接和缺失的本地目标；未完成任务只提醒，不会被删掉。取消导出不会改动原文件。
...
- 工作区搜索结果可以插入到当前文章：带来源链接的片段是插入当时的快照，不会改来源文件；换成另一篇后再插入会被拒绝。
