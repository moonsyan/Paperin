...
当前生产版启动时会通过 `electron-updater` 联系配置的 GitHub 发布源检查更新，并自动下载可用更新。代码把 `autoDownload` 设为 `true`，但没有设置 `autoInstallOnAppQuit`；该库默认值为 `true`，因此已下载的更新会在应用退出时安装。检查、下载和退出安装都没有用户可见开关，该过程不应上传 Markdown 正文。图片默认保存为本地附件；只有用户主动配置并选择 SM.MS 图床后，粘贴或拖入的图片才会发送到 SM.MS。SM.MS token 虽然只由主进程读写，但当前仍以明文保存在本机 `userData/settings.json`，尚未接入操作系统安全存储。拼写检查默认关闭；用户在设置中打开后，Electron 可能下载对应语言词典，这不上传正文。外部 Alpha 前必须先让更新检查、下载和退出安装都可关闭，把 token 迁入操作系统安全存储，再按这三类联网路径补齐独立隐私说明。
