...
- 找回与复用：快速打开、工作区全文搜索、当前文档查找替换、最近编辑、收藏、标签、Wiki 链接、反向链接和关系图谱。工作区搜索命中后可「插入引用」，保存重开后引用仍在，并可导出 HTML 资源包。
