...
- Markdown 写作：Milkdown/ProseMirror 编辑器支持 GFM、任务列表、表格、代码、公式、Mermaid、脚注和 frontmatter。从其他软件复制 Markdown 原文时，会按标题、列表和强调排版，不会因为剪贴板附带的 HTML 变成纯文本。
- JSON 与 YAML 代码块可以在块内格式化或压成一行。有缩进的代码块可以折叠其中一段，折叠不改文件里的文字。
- Mermaid 图在渲染前会去掉不影响结构的缩进、零宽字符和会改全局样式的 init 指令。合法图不再一律标成语法错误；画不出来时可以切回源码再看图表。
