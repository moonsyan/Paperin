...
Paperin 是本地优先的 Markdown 写作与知识工作台，让文章、笔记和项目资料在同一个空间中持续积累、相互复用，并形成可维护、可交付的成果。短表达：**安心写作，让知识持续产生作品。** 本地 Markdown 知识工作台：收下资料，写出理解，带来源交付。首批可演示任务仍是：打开自己的资料，完成一篇有来源、可继续维护的成果。

Paperin 是项目统一产品名称，工作区状态写入 `.paperin`，应用设置和缓存沿用 Electron 的 Paperin 用户数据目录。

当前本地版本为 **0.7.0 种子验证候选**；GitHub 已有 v0.6.0 公开发行记录。代码能力、自动门禁、安装验证与真人任务分别计分，最新结果见 [项目状态](docs/PROJECT-STATUS.md)。逐篇来源基线、重定位、索引失效、缓存校验和有界搜索语料已落地；2026-09-23 审查另外复现了状态目录越界、来源身份/路径及测试/性能门禁问题，修复前不能将候选视为稳定公开版。

## 产品方向

产品面向需要长期积累资料、写作和管理个人项目的用户。通用写作、个人知识管理与项目文档共用同一套工作区；早期推广仍可用「从旧资料完成一篇有来源的成果」作为示范任务。设计原则是“安静写作，随时复用”。默认页面突出当前文档，低频操作集中到“更多”、命令面板和上下文面板；知识库、文件树、标签、反向链接和图谱仍然可用，但不会同时抢占正文的注意力。

当前阶段保持免费，优先把本地写作、资料复用和可靠性做好。默认不启用账号和云同步。HTML 资源包发布、来源变化提示和交付报告已经在本地实现；远程 AI、账号同步和跨设备服务仍未做，也不把研究方案当成已上线功能。

核心闭环示意（首批可演示任务）：打开资料 → 用来源写一段技术说明。

```text
打开资料 → 找到相关内容和来源 → 写成当前结论 → 安全保存并继续维护 → 交付成果
```
...
来源健康已按文章维护独立基线，旧版全局记录保留为归属未知；最多跟踪 50 篇文章、每篇 20 条、总计 200 条。显式重新定位可迁移关系，并选择是否可撤销地更新普通 Markdown 链接。提示依据相对路径与修改时间，不代表正文已人工核验；草稿切换及首次保存的来源归属缺陷见 [本次审查](docs/development/reviews/2026-09-23-product-state-audit.md)。
...
首次启动会显示合成「示例任务 / 资料来源」文件夹（旧笔记、技术说明草稿与可引用来源），并完成一篇欢迎引导；**示例不会写入**你打开的知识库。欢迎正文区分「先熟悉界面」与「开库后的完整闭环」：无库时演示树用不了全文搜索和插入引用，保存会另存为。关闭全部标签会出现开始页；有知识库且「最近编辑」含本库文件时主按钮为「继续最近编辑」。桌面系统中从文件管理器打开 Markdown 时，Paperin 使用居中的普通应用窗口；保存已有文件会保留文件对象，不会主动移动桌面图标。
- “更多…”只呈现高频快捷动作和按任务分组的低频能力；菜单项、快捷键和命令面板共用同一套命令注册表。
- 命令面板可以新建六类普通 Markdown 模板：README、API 文档、设计文档、更新日志、技术文章、决策记录。模板只是普通 Markdown，不会覆盖已打开的文件，也不要求署名。
...
正文始终是用户拥有的本地 Markdown 文件。应用设置、标签页、草稿、最近文件、收藏和统计与正文分离；收藏按工作区路径分桶写入本机设置。应用不要求登录，不会默认上传正文。支持摘要采用脱敏白名单；本次静态审查发现主进程异常日志仍直接保存错误文本与堆栈，可能包含路径，需修正后才能承诺日志不含绝对路径。
...
- [2026-09-23 产品战略发展报告](docs/PRODUCT-STRATEGY-REVIEW-2026-09-23.md)：结合代码、定向复现、工程检查和官方竞品资料，覆盖通用写作、个人知识管理、项目文档，以及个人到外部、团队与企业的阶段路线。
- [产品整体工作流](docs/PRODUCT-WORKFLOW.md)：从发现安装、资料写作、保存恢复到交付维护的端到端产品契约。
- [按优先级排列的完整实施计划](docs/superpowers/plans/2026-09-22-product-workflow-implementation.md)：计划契约与勾选；完成状态以项目状态为准。
- [当前项目状态](docs/PROJECT-STATUS.md)：当前代码能力、新鲜门禁结果、红灯、未验证范围和执行顺序。
- [2026-09-23 状态审查证据](docs/development/reviews/2026-09-23-product-state-audit.md)：A01–A10 与本机门禁数字。
...
- [`docs/graph-view-architecture.md`](./docs/graph-view-architecture.md)：关系图谱维护边界与索引新鲜度依赖。
...
- [`docs/development/`](./docs/development/)：性能基线、发行验证、导出接收方矩阵、主题基线和回归脚本数据。

`docs/IMPLEMENTATION-PLAN.md` 是稳定开发入口，详细任务以 2026-09-22 产品工作流实施计划为准。被替代的审计和计划由 Git 历史保留，不在当前树重复维护。

## 当前边界与后续方向

已有工程能力包括已有文件的中断恢复、快照超时不写旧版本、关闭前不放行未确认内容、逐篇来源维护、可复用发布配置和脱敏交付报告。当前验证、性能与新缺陷以 [项目状态](docs/PROJECT-STATUS.md) 和 [审查证据](docs/development/reviews/2026-09-23-product-state-audit.md) 为准，不以旧性能红灯或测试绿灯替代本次结果。Windows 安装/升级/卸载循环、8 小时稳定性和第二台设备仍为 **UNVERIFIED**。三平台安装验证、真实输入法、缩放和权限异常不能用一次开发态构建替代。

下一步优先修复工作区状态目录与来源身份/路径边界，恢复默认测试和性能门禁，再完成 Windows 安装态验收并观察两位现有用户在写作、知识整理和项目资料中的真实任务。GitHub `moonsyan/Paperin` 与本地发布元数据已对齐，公开旧版可访问；当前候选 Draft 与远端 CI 状态未验证。远程 AI 资料问答和跨设备同步仍未实现；发布扩展从已有本地导出和交付报告开始，保留原始 Markdown 的可取回性。

研究先进行两人种子观察和外部小样本发现。12 人 U01/U02 与两批 W2/W4 用于资料复用主张及商业扩张确认；通用写作和知识找回分别定义任务成功，不强制引用或导出。安全与发行门禁保持独立，没有研究样本时保持未验证，详见最新战略。
