...
| P1 | S06 五分钟激活与兼容试开 | 部分完成 | 打开知识库后报告未扫完、缺附件、断链和残缺脚注，且不改原文；用户任务成功率与来源夹具 hash 未验证 |
| P1 | S07 中文搜索与覆盖解释 | 部分完成 | 文件名和标题优先的可解释排序，范围、相对目录、匹配上限和未扫完分开说明；Hit@5 与 5000 文件时延未验证 |
| P1 | S08 搜索结果到当前文章 | 部分完成 | 插入来源快照后回到打开搜索时的位置，切文档后拒绝旧结果；用户任务耗时改善未验证 |
| P1 | S09 跨会话恢复与回访 | 部分完成 | 文档视图已恢复阅读位置；工作区设置记住上次搜索词，缺字段默认为空。真实重启、改名和试用者回访未验证 |
| P1 | S10 核心导出与预检 | 部分完成 | Markdown、HTML、PDF、Word 和 Pandoc 导出前检查空图片、不安全链接和缺失本地目标；未完成任务只提醒。多格式平台人工检查未完成 |
