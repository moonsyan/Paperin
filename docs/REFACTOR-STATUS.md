...
2026-09-19 夜间再次执行 `npm run typecheck` 和 `npm run test`（193 个测试文件、1397 项通过）。同日较早一次 `npm run build` 已通过；这一轮没有重跑 `build`、`lint`、`a11y`、`smoke` 或 `perf:electron`。
