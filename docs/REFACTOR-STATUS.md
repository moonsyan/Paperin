...
**2026-09-12 M2 起步（T10）**：按 [UI 规范](NEXT-UI-SPEC.md) §3 完成顶栏、路径条与导航层级——正文上方新增 28px 轻路径条（示例/未命名/库内/外部四态，目录段与「定位到文件」可展开侧栏并展开祖先链）、标签栏同名文件相对目录消歧、库名常驻展示收敛到侧栏标题（侧栏收起时顶栏才显示）。全量回归 146 文件 1173 项通过。
