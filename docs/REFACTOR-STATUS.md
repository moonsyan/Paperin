...
关闭保存现由 `useDocumentCloseSaving` 统一处理。它在请求开始时绑定文件路径、活动会话序号与编辑器实例，并在快照等待、关闭确认、另存为、队列写入后再次核对身份、正文和编辑器未落账输入；不满足时保留文档并拒绝关闭。窗口关闭在逐标签确认之后，还会在队列和工作区视图持久化完成后复核全部标签，因此等待期间的新输入、新标签、路径迁移或未落账输入不会继承旧的关闭许可。

自动回归新增未命名 5 MiB 文档、另存为期间继续输入、异步另存为失败/卸载、A→B→A 会话切换，以及 32 字符、1 MiB 阈值两侧和 5 MiB 四种大小各 30 次“保存中继续输入”场景；窗口关闭覆盖队列/工作区写入等待、新标签、路径变更和未落账输入。本次 `npm run test`（166 文件、1271 项）、lint、a11y、typecheck、build 和真实 Electron smoke 通过。完整 Q02 仍未完成：真实中文 IME、工作区切换、磁盘结果组合、每类全量 30 次和跨平台人工验证尚待执行。
