...
大文档保存现在捕获目标文件 ID 与活动编辑会话序号。`ensureFreshSnapshot` 在等待期间发现超时、标签/工作区切换，或切换后又回到同一文件 ID 时，返回带原因的未完成结果；手动保存不会把旧缓存写入目标路径，关闭流程不冲刷队列且返回拒绝关闭。普通编辑器同步读取保持原有低延迟路径；大文档已落账快照不会在异步等待后反向覆盖随后到达的内容。
...
性能冒烟现在记录快捷键分发时间、Renderer 调用 `document.save` 的时间与内容长度、以及 IPC 回执时间；没有保存回执时，15 秒后带阶段证据失败，不再额外等待磁盘标记 120 秒。该改动不会放宽 5 MiB 保存硬门禁，也不把未调用保存视为成功。

自动验证覆盖“只有成功保存回执才轮询磁盘标记”；类型检查、lint、构建和普通 Electron smoke 通过。未完成：记录新的受控 5 MiB 运行结果，定位并修复瓶颈，验证尾部与末次输入落盘、5 MiB 打开/保存/导出和 20 标签切换门槛，以及 M01 的多批次性能与长运行内存要求。
