...
整个产品重构尚未完成。系统文件关联、多窗口规则、真实生产路径性能、完整 quiet-workspace 迁移、历史大文件拆分和三平台安装包验证仍是明确的发布前工作。
...
- 固化系统文件关联、外部临时文件、同一路径多窗口去重、关闭外部文件和关闭确认规则，并增加 Electron smoke 或 Playwright 端到端测试。
- 建立生产路径性能门禁：直接覆盖 `WorkspaceIndexService`、增量索引、文件监听风暴和真实搜索 IPC。
...
- `App.tsx`、GraphView、Sidebar、编辑器 hooks 和部分 Main IPC 文件仍超过项目行数门禁；本批新组件已拆分，但历史巨型文件需要按控制器、视图和副作用边界继续拆解。
