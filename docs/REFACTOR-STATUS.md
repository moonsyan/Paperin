...
当前代码可通过类型检查、lint、1140 项测试和构建，生产索引/搜索/监听门禁通过。**2026-09-12 M0 轮已恢复可信验证入口**：Electron smoke 的过时 `.current-file-banner-title` 契约已替换为「活动标签名 + 外部来源 + 正文标记」联合断言，smoke 完整链路（关联 → 工作区 → 新建 → 保存 → 冲突 → 重读 → 重命名 → 搜索 → 状态读取）实测通过；收藏重启恢复、侧栏快捷键提示同源与文档计数也已修复并有行为测试；CI 配置已恢复并通过本地结构校验（实际 CI 运行证据仍待托管平台支持）。
