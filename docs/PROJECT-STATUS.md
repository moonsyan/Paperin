...
目前不能称为稳定公开版。生产依赖审计、许可和安装循环仍有阻断或缺口；两位现有用户的任务记录尚未采集。
...
1. 消除生产依赖高危漏洞。
2. 清理失效 `demo:soft*` 脚本。
3. 让更新检查、自动下载和退出安装可关闭，并把 SM.MS token 迁入系统安全存储。
4. 确认许可和资源权利，按三类联网路径编写隐私说明，完成 Windows 候选安装循环。
5. P0 退出后完成两位现有用户两周任务观察，只修最大的真实阻塞；该观察不阻塞 P0。
6. 再进入 GitHub 外部 Alpha，并在发布材料中补 `CHANGELOG.md` 与 `CONTRIBUTING.md`。个人重复价值成立后才验证付费；至少 3 个团队重复提出同类问题后才进入团队方案。
