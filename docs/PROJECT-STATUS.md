...
| 发行 | 三平台构建配置、Draft 候选和显式正式发布门禁；更新检查/下载/退出安装可由设置关闭 | Windows 安装循环、macOS 签名/公证、Linux 桌面集成；独立隐私与安全响应说明尚未建立 |
...
1. 确认许可和资源权利，按三类联网路径编写隐私说明，完成 Windows 候选安装循环。
2. P0 退出后完成两位现有用户两周任务观察，只修最大的真实阻塞；该观察不阻塞 P0。
