...
更新时间：2026-09-23（Asia/Shanghai）。2026-09-23 审查重跑 `npm run a11y` 退出 0：九主题 254 项检查、37 项基线豁免、0 失败；焦点扫描无违规。下文早期主题表和修复过程是历史证据，不作为当前数量。代码里的输入法 Escape 保护不能代替真机中文输入法、全键盘和缩放人工检查；设置与版本历史已接入统一焦点闭环（A09 代码侧已收口），静态 a11y 通过仍不等于真机弹窗矩阵全绿。
