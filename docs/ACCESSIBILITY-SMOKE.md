...
更新时间：2026-09-23（Asia/Shanghai）。2026-09-23 审查重跑 `npm run a11y` 退出 0：九主题 254 项检查、37 项基线豁免、0 失败；焦点扫描无违规。下文早期主题表和修复过程是历史证据，不作为当前数量。代码里的输入法 Escape 保护不能代替真机中文输入法、全键盘和缩放人工检查；部分弹窗尚未接入统一焦点闭环（审查 A09），静态 a11y 通过不等于弹窗行为全绿。
