...
> 2026-09-15 S03 复核：真实 Electron 硬门禁已在同一 Windows 开发机、同一 5 MiB 长段落夹具上独立运行三次并全部通过。性能脚本直接以磁盘 Markdown 验证原文尾部和末次输入；Milkdown 对正文下划线的 `\_` 转义与原文视作同一文本。从保存快捷键到磁盘确认直接计时，不再包装不可写的 preload bridge 或额外等待 15 秒。完整 M01 多结构、多设备和长运行验收仍未完成。

> 2026-09-14 历史复核说明：本文保留既有夹具、历史样本与阈值。当时 S03 尚在增加保存阶段诊断，5 MiB 门禁未记录为通过；结论已由上方 2026-09-15 的实测更新。另，文中“超过 1 MiB 优先快照”的实现实际为 `content.length > 1_000_000`（字符串长度），不是字节数。最新状态见 [REFACTOR-STATUS](../REFACTOR-STATUS.md)，顺序见 [实施计划](../NEXT-DEVELOPMENT-PLAN.md)。
