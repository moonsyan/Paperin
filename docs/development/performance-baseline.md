...
> 2026-09-12 复核说明：本文保留既有夹具、历史样本与阈值。最新生产模块性能测试通过，但普通 Electron smoke 因顶栏旧选择器失败，大文档性能入口共用该前置流程，本轮未运行。另，文中“超过 1 MiB 优先快照”的实现实际为 `content.length > 1_000_000`（字符串长度），不是字节数；快照新鲜度仍待验证。最新结果与改进方向见 [项目评估](../NEXT-PRODUCT-ASSESSMENT.md) 和 [实施计划](../NEXT-DEVELOPMENT-PLAN.md)。
...
240 秒内仍未进入 `document.save` IPC，磁盘保持原始内容。现在保存/关闭路径对超过 1 MiB
文档优先复用已落账的编辑快照，避免重复同步序列化；修复后的完整 Electron 门禁仍需在
稳定构建上重新跑完，未提前记录为“通过”。
