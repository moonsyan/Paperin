...
2026-09-10 当前 Windows 开发机实测中，5 MiB 编辑后的 Milkdown Markdown 序列化在
240 秒内仍未进入 `document.save` IPC，磁盘保持原始内容，因此该 Electron 门禁当前为
失败状态；这是真实产品性能问题，不应被记录为“通过”。打开/编辑步骤和普通 smoke 已通过，
后续应优化大文档序列化或按块保存后再恢复该全链路门禁。
