...
> 2026-09-15 复核补充：S01/S02 已完成保存恢复、快照超时和关闭前旧缓存保护的首批工程回归；S03 改用 Electron 原生 Ctrl/Cmd+S 输入。`document.save` 的渲染层包装只在 preload bridge 可写时作为阶段诊断；不可写时报告 `SAVE_OBSERVER_UNAVAILABLE` 并继续按磁盘标记执行原有硬门禁。5 MiB 硬门禁仍未记录为通过。

> 2026-09-14 复核说明：本文保留既有夹具、历史样本与阈值。S01/S02 已完成保存恢复和快照关闭保护的首批工程回归；S03 已为真实 Electron 保存门禁增加快捷键、`document.save` 调用和回执阶段诊断。5 MiB 硬门禁仍未记录为通过，性能瓶颈和多批次 M01 采样待完成。另，文中“超过 1 MiB 优先快照”的实现实际为 `content.length > 1_000_000`（字符串长度），不是字节数。最新状态见 [REFACTOR-STATUS](../REFACTOR-STATUS.md)，顺序见 [实施计划](../NEXT-DEVELOPMENT-PLAN.md)。
...
2026-09-10 当前 Windows 开发机实测中，修复前 5 MiB 编辑后的 Milkdown Markdown 序列化在
240 秒内仍未进入 `document.save` IPC，磁盘保持原始内容。当前保存/关闭路径对超过 1 MiB
文档优先复用已落账的编辑快照；S02 又增加了超时不写旧缓存和会话改变不放行关闭的保护。
完整 Electron 门禁仍需在稳定构建上重新采样，未记录为“通过”；S03 的阶段诊断只改善定位，不改变性能阈值。

## 尚未覆盖

2026-09-15 复核确认：context bridge 对象可能不可写，旧脚本对 `document.save` 的包装不能单独证明 IPC 未调用。历史“未进入 IPC”表述只保留为当时的观察，后续以主进程阶段埋点或磁盘确认补齐分段结论。
