...
### T19 / P1：产品名称与安装身份兼容决策

建立 ADR，分别列出 UI 名称、package name、appId、userData、`.markdownsoft` 工作区状态目录、文件关联、更新源和安装包命名。默认优先保持已有存储/安装身份兼容；是否迁移必须有可检测的旧版本与回退方案。

测试旧配置/草稿/工作区打开、升级后关联、重复启动、多窗口、卸载后用户文档保留。不能通过全局文本替换完成品牌迁移，也不能把旧发布仓库直接当作新产品发布目标。依赖 T04、T14；约 1–2 日。
