...
**当前来源边界：** `sourceSnapshots` 是全工作区共用的最多 50 条记录，同来源路径再次登记会覆盖时间；质量面板没有逐篇文章独立基线。“重新定位”当前只打开来源文件名搜索；清除导航同时清除搜索词、最近引用及这组快照，正文不变。P1-06 的异步隔离、P1-07 的逐篇复核及 P2-02 的显式迁移均未完成，不能把计划动作写入当前命令列表。可预览支持摘要 P1-04 也尚无可用命令。
