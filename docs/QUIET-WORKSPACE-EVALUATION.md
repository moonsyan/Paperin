...
> **2026-09-12 迁移进展**：P1 三项已落地——顶栏四层 chrome 收敛为单条 52px 三区顶栏、侧栏改为五段式（搜索触发框 / 快捷导航 / 集合标题 / 文件树 / 底部区，收藏数据层已补）、新增「雾白」「夜松」两套留白主题并把排版尺度落到主题作用域样式；P3 的全局 `prefers-reduced-motion` 已补齐。仍未完成：窄窗口侧栏与 ContextDock 共用 scrim、AppearancePanel 主题样张卡、ContextDock 窄栏极简模式。详见 `REFACTOR-STATUS.md`。
