...
| 模型 | 当前版本/内容 | 限制 |
| --- | --- | --- |
| `WorkspaceSettingsState` | schema 1；主题、附件目录、查询、最近引用、来源时间、发布配置 | 最近引用 8 条、来源快照 50 条、发布配置 20 条 |
...
应用全局设置、草稿、版本历史、索引缓存位于应用私有数据目录，详见[隐私说明](../PRIVACY.md)。清除导航当前会清除查询、最近引用和共享来源快照，不删除 Markdown。

## 来源与索引的待实施边界

当前 `SourceSnapshot` 仅有 `path/modifiedTime`，同来源全工作区共用一份时间，重复登记会覆盖；不含引用文档身份。`evaluateSourceHealth` 依据 mtime 和覆盖派生 current/changed/missing/unverified，current 只表示时间一致。

| 任务 | 目标契约 | 迁移要求 |
| --- | --- | --- |
| P1-06 | 请求绑定工作区 epoch/记录版本，切库/清除/卸载使旧回包失效 | 两个入口仅凭实际插入成功登记；失败不回滚正文 |
| P1-07 | 每篇引用文档独立来源基线与明确复核 | 旧记录保留为归属未知；定义未保存/另存为/移动及配额；不批量确认 |
| P0-07 | 正文解析与引用目标存在性分别失效 | 不改正文也反映目标变化；不原地修改已发布索引 |
| P0-02 | Main-only 有界搜索语料 | 结构索引可缓存，正文语料不落盘；驱逐后安全回退并保留 coverage |
| P1-08 | 队列、订阅、cache writer 共用生命周期约束 | 缓存有界读取、schema/根校验、损坏重建；旧代次不能覆盖新根 |

以上目标尚未落地，本次文档更新不修改 schema/IPC。实施时 Shared DTO、Main store、Preload、Renderer、直接测试与迁移说明同提交；未来 schema 不能被旧代码静默覆写。详细文件与验收见[实施计划](superpowers/plans/2026-09-22-product-workflow-implementation.md)。
