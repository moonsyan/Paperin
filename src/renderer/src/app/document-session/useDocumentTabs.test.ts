...
describe('切换标签等待大文档快照', () => {
  ...
  it('新建文档时大文档仍有未落账输入则不替换编辑器', async () => {
    vi.useFakeTimers()
    const replaceEditorContent = vi.fn()
    const setToast = vi.fn()
    const large = 'x'.repeat(1_000_001)
    ...
    const { result } = renderHook(() => {
      const state = useDocumentState()
      state.openFilesRef.current = [{ id: 'file-a', name: 'a.md', path: 'D:/notes/a.md' }]
      state.contentsRef.current = { 'file-a': large }
      state.activeFileIdRef.current = 'file-a'
      return useDocumentTabs({
        state,
        editorRef: { current: editor },
    ...
    const pending = result.current.handleNew()
    await vi.advanceTimersByTimeAsync(SNAPSHOT_SETTLE_TIMEOUT_MS)
    await pending
    expect(replaceEditorContent).not.toHaveBeenCalled()
    expect(setToast).toHaveBeenCalledWith('大文档仍有未落账输入，已取消操作')
  })
})
