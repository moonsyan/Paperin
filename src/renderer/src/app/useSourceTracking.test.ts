...
describe('useSourceTracking 身份迁移 A02/A03', () => {
  const renderIdentity = (initial: { citing: string; docId: string }) => {
    const statQueue: StatDeferred[] = []
    stubDesktopStat(statQueue)
    const rendered = renderHook(
      ({ citing, docId }: { citing: string; docId: string }) => {
        const [settings, setSettings] = useState<WorkspaceSettingsState>(DEFAULT_WORKSPACE_SETTINGS)
        const tracking = useSourceTracking({
          workspacePath: WORKSPACE_A,
          citingDocumentKey: citing,
  ...
  it('A02：草稿登记来源后切换到另一篇库内文章，不得迁给 B', async () => {
    const { result, rerender, statQueue } = renderIdentity({
    ...
    await waitFor(() => {
      expect(result.current.ephemeralBaselines).toEqual([
        { citingDocumentPath: '@unsaved:draft-a', sourcePath: '资料/source.md', modifiedTime: 99 },
      ])
    })

    await act(async () => {
      rerender({ citing: '文章/b.md', docId: 'file-b' })
    })
  ...
  it('A03：stat 晚于首次保存回包时归属保存后的库内路径', async () => {
    const { result, rerender, statQueue } = renderIdentity({
      citing: '@unsaved:draft-a',
      docId: 'draft-a',
    })
    ...
    await act(async () => {
      result.current.commitCitingIdentityMigration('@unsaved:draft-a', '文章/a.md')
      rerender({ citing: '文章/a.md', docId: 'file-a' })
    })

    await act(async () => {
      deferred.resolve({ ok: true, data: { modifiedTime: 77 } })
    })

    await waitFor(() => {
      expect(result.current.settings.editor.documentSourceBaselines).toEqual([
        { citingDocumentPath: '文章/a.md', sourcePath: '资料/source.md', modifiedTime: 77 },
      ])
    })
    expect(result.current.ephemeralBaselines).toEqual([])
  })
