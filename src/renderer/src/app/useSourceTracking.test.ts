...
describe('useSourceTracking.rememberSourceAfterInsert', () => {
  it('stat 成功后写入来源快照', async () => {
    ...
    await waitFor(() => {
      expect(result.current.settings.editor.sourceSnapshots).toEqual([
        { path: '资料/source.md', modifiedTime: 99 },
      ])
    })
  })
  ...
  it('乱序回包时仅仍有效的请求会写入', async () => {
    ...
    await waitFor(() => {
      expect(result.current.settings.editor.sourceSnapshots).toEqual([
        { path: '资料/source.md', modifiedTime: 20 },
      ])
    })
  })
