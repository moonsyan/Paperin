...
import * as editorOverlays from './useEditorOverlays'
...
describe('Wiki 链接自动补全候选筛选', () => {
  it('同一选区重渲染时复用首次计算的滚动容器内容坐标', () => {
    const getWikiAutocompletePosition = (editorOverlays as {
      getWikiAutocompletePosition?: (options: {
        from: number
        to: number
        ...
        cached: { key: string; x: number; y: number } | null
      }) => { key: string; x: number; y: number }
    }).getWikiAutocompletePosition

    expect(getWikiAutocompletePosition).toBeTypeOf('function')
    expect(
      getWikiAutocompletePosition?.({
        from: 4,
        to: 9,
        coords: { top: 180, left: 140 },
        anchor: {
          scrollTop: 30,
          scrollLeft: 20,
          getBoundingClientRect: () => ({ top: 100, left: 80 }),
        },
        cached: { key: '4:9', x: 60, y: 110 },
      }),
    ).toEqual({ key: '4:9', x: 60, y: 110 })
  })
