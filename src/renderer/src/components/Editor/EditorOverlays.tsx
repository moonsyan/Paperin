...
export type SelectionFormatKind = 'bold' | 'italic' | 'strike' | 'code'
...
interface EditorOverlaysProps {
  ...
  onSelectionFormat: (kind: SelectionFormatKind) => void
    ...
    <>
      ...
      {tablePanel && (
        <div
          ...
          contentEditable={false}
        >
          <button type="button" className="table-act" onClick={() => onTableAction('addRow')} title="在光标所在行下方加行">+行</button>
          <button type="button" className="table-act" onClick={() => onTableAction('addCol')} title="在光标所在列右侧加列">+列</button>
          <button type="button" className="table-act" onClick={() => onTableAction('delRow')} title="删除光标所在行">−行</button>
          <button type="button" className="table-act" onClick={() => onTableAction('delCol')} title="删除光标所在列">−列</button>
          <button type="button" className="table-act danger" onClick={() => onTableAction('delTable')} title="删除整个表格">删表</button>
          <span className="table-act-sep" />
          <button type="button" className="table-act" onClick={() => onTableAction('alignLeft')} title="当前列左对齐">左</button>
          <button type="button" className="table-act" onClick={() => onTableAction('alignCenter')} title="当前列居中">中</button>
          <button type="button" className="table-act" onClick={() => onTableAction('alignRight')} title="当前列右对齐">右</button>
        </div>
      )}
      ...
      {selectionToolbar && (
        ...
        >
          <button
            type="button"
            className="sel-act"
            onClick={() => onSelectionFormat('bold')}
            title="粗体（Ctrl+B）"
            aria-label="粗体"
          >
            <strong>B</strong>
          ...
          <button
            type="button"
            className="sel-act"
            onClick={() => onSelectionFormat('italic')}
            title="斜体（Ctrl+I）"
            aria-label="斜体"
          >
            <em>I</em>
          ...
          <button
            type="button"
            className="sel-act"
            onClick={() => onSelectionFormat('strike')}
          ...
          <button
            type="button"
            className="sel-act"
            onClick={() => onSelectionFormat('code')}
            title="行内代码"
            aria-label="行内代码"
          >
            {'</>'}
          ...
          <div className="code-fullscreen" onClick={(event) => event.stopPropagation()}>
            <div className="code-fullscreen-head">
              ...
              <div className="code-fullscreen-actions">
                <button type="button" className="dialog-btn" onClick={onCopyFullscreen}>复制</button>
                <button type="button" className="dialog-btn" onClick={onCloseFullscreen}>关闭（Esc）</button>
              </div>
            </div>
