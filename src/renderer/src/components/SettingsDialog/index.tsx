...
export function SettingsDialog({
  ...
  onThemeChange,
  workspaceAvailable,
  workspaceThemeEnabled,
  onWorkspaceThemeEnabledChange,
  fontSize,
  onFontSizeChange,
  contentWidth,
  ...
  const matchedSettings = useMemo(() => {
    ...
    return SETTINGS_SEARCH_INDEX.filter(
      (item) =>
        item.label.toLowerCase().includes(q) || item.panel.includes(q),
    )
  }, [searchQuery])
          ...
          {matchedSettings ? (
            <>
              {matchedSettings.length === 0 && (
                <div className="settings-search-empty">无匹配设置项</div>
              )}
              {matchedSettings.map((item) => (
                <button
                  type="button"
                  key={item.label}
                  className="dialog-nav-item settings-search-hit"
                  onClick={() => {
                    setNav(item.panel)
                    setSearchQuery('')
                  }}
                  title={`跳转到「${NAV_ITEMS.find((n) => n.id === item.panel)?.label ?? item.panel}」`}
                >
                  {item.label}
                </button>
              ))}
        ...
        <div className="dialog-body">
          ...
          {nav === 'appearance' && (
            <AppearancePanel
              ...
              workspaceThemeEnabled={workspaceThemeEnabled}
          ...
          {nav === 'editor' && (
            <EditorPanel
              ...
              hasWorkspace={workspaceAvailable}
            />
          )}
