...
describe('文档标签状态', () => {
  ...
  it('固定后移到固定分组末尾，取消固定后移到普通分组开头', () => {
    const files = [
      { id: 'pinned-a', name: 'A.md', pinned: true },
      { id: 'b', name: 'B.md' },
      { id: 'c', name: 'C.md' },
    ]

    const pinned = togglePinnedTab(files, 'c')
    expect(pinned.map((file) => [file.id, file.pinned === true])).toEqual([
      ['pinned-a', true],
      ['c', true],
      ['b', false],
    ])

    const unpinned = togglePinnedTab(pinned, 'pinned-a')
    expect(unpinned.map((file) => [file.id, file.pinned === true])).toEqual([
      ['c', true],
      ['pinned-a', false],
      ['b', false],
    ])
  ...
  it('关闭其他保留右键目标和所有固定标签', () => {
    ...
    expect(getClosableTabIds(files, 'others', 'c')).toEqual(['b'])
    expect(getClosableTabIds(files, 'all', null)).toEqual(['b', 'c'])
  })

  it('拖拽不能跨越固定和普通标签分组', () => {
    const files = [
      { id: 'pinned-a', name: 'A.md', pinned: true },
      { id: 'pinned-b', name: 'B.md', pinned: true },
      { id: 'c', name: 'C.md' },
    ]

    expect(reorderTabsWithinGroup(files, 0, 1).map((file) => file.id)).toEqual([
      'pinned-b',
      'pinned-a',
      'c',
    ])
    expect(reorderTabsWithinGroup(files, 0, 2)).toBe(files)
  })

  it('程序性载入后的规范化内容仍视为已保存', () => {
    const initialized = initializeDocumentBaseline('# 标题\n', '# 标题')
