...
export function replaceFrontmatter(
  ...
  if (newYaml) {
    // 换行符随原文档（CRLF 文档整体改写后不得混入 LF，见 setFrontmatterProperty 同口径）
    const eol = getLineBreak(markdown)
    const replacement = `---${eol}${newYaml.replace(/\n/g, eol)}${eol}---`
    if (existing) {
      return replacement + markdown.slice(existing.end)
    }
    return replacement + eol + eol + markdown
  }
...
/** 统计一行 YAML flow 语法（[ ] / { }）的括号深度增量；引号内括号不特殊处理（保守） */
const flowDepth = (line: string): number => {
  let depth = 0
...
export function deleteFrontmatterProperty(markdown: string, key: string): string {
  ...
  // L10：值非空的行内属性只删键行即可；值为空（块序列 `tags:\n  - a`）
  // 或块标量（`|`/`>` 多行）时必须连后续缩进块一起删——只删键行会留下
  // 孤立缩进行，把 frontmatter 顶层变成序列文档、破坏结构
  const keyLine = lines[index]
  const inlineValue = /^[A-Za-z0-9_-]+[ \t]*:[ \t]*(.*)$/.exec(keyLine)?.[1]?.trim() ?? ''
  if (!inlineValue || /^[|>]/.test(inlineValue)) {
    // 吞并后续缩进块；YAML 块标量体可合法包含空行（多段文本），
    // 空行后仍出现缩进行时一并属于本属性，否则中途停止会留下孤行
    let end = index + 1
  ...
  } else if (/^[[{]/.test(inlineValue) && flowDepth(inlineValue) > 0) {
    // 多行 flow 序列/映射（`tags: [` 跨行到 `]`）：后续行无论缩进与否
    // 都属于该值，按括号配平吞并——只删键行会留下孤立的 `  - a` 与 `]`
    let depth = flowDepth(inlineValue)
    let end = index + 1
    while (end < lines.length && depth > 0) {
      depth += flowDepth(lines[end])
      end++
    }
    lines.splice(index, end - index)
  } else {
