...
const desktopAPI = {
  ...
  document: {
    ...
    /** 保存到指定路径（expectedMtime + expectedContentHash 绑定本编辑器读取版本；forceOverwrite 表示用户已确认覆盖） */
    save: (
      path: string,
      content: string,
      ...
      ipcRenderer.invoke(CHANNELS.FILE_SAVE, {
        path,
        content,
        expectedMtime,
        encoding,
  ...
  settings: {
    ...
    upsertDraft: (
      ...
      ipcRenderer.invoke(CHANNELS.SETTINGS_UPSERT_DRAFT, {
        id,
        content,
        baselineSha256,
        draftSessionId,
      }),
    deleteDraft: (id: string, draftSessionId?: string) =>
