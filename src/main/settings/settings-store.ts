...
async function backupCorrupt(path: string): Promise<void> {
  try {
    ...
    // 超限/损坏的 settings 每次读取都会走到这里（自愈要等下一次写入），
    // 只读不写的启动会无限累积 40MB 级备份——保留最新一份，其余清理
    const dir = dirname(path)
    const base = path.split(/[\\/]/).pop() ?? 'settings.json'
    const backups = (await readdir(dir))
      .filter((name) => name.startsWith(`${base}.corrupt-`) && name.endsWith('.bak'))
      .sort()
